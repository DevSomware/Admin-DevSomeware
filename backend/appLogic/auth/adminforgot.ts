...
export const Adminforgot = async (req:any, res:any) => {
try{
...
const transporter = nodemailer.createTransport({
  ...
  if(user==null){
    return res.status(400).json({message:"Admin not exists cannot forgot your password",success:false});
  }
  //if exists create token send it back to user via email
  let token = jwt.sign({id:user._id,role:user.role,email:user.email,name:user.name},process.env.JWT_SECRET||"");
  ...
  const info = await transporter.sendMail({
    ...
    subject: "Devsomeware: Admin Password Reset Request", // Subject line
...
<body>
  ...
  return res.status(200).json({message:"Reset password link sent to your email",success:true});
    ...
    return res.status(500).json({message:`Error in Adminforgot ${err}`,success:false});
}
